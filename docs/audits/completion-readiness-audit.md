...
- Browser proof: `pnpm test:e2e:windows test tests/version-skew.spec.ts --project=chromium --reporter=list`
- Service-worker proof: `pnpm test:e2e:windows test tests/service-worker.spec.ts --project=chromium --reporter=list`
- Article proof: `pnpm test:article` or `pnpm test:article:windows` on Codex/WSL, including the readable browser walkthrough in `tests/learning-path.spec.ts`
- Learning sweep: `pnpm test:learning:windows`, adding contract, privacy, policy, accessibility, and complexity checks
- Focused accessibility proof: `pnpm test:e2e:windows test tests/accessibility.spec.ts --project=chromium --reporter=list`
...
- Compact browser regression: `pnpm test:e2e:windows test tests/version-skew.spec.ts --project=chromium --reporter=list` passed 10 tests.
...
| --- | --- | --- | --- |
| Standalone React/Vite/TypeScript lab, no Next.js, fake data only | Proven | `package.json`, `README.md`, `server/mock-data.ts`, `tests/project-contract.spec.ts` | The stack and docs keep the POC standalone and fake-data only. |
| Plain lab examples | Proven | `src/pages/*`, `src/workflows/*`, `server/skew-server.ts`, `server/mock-data.ts` | Start, examples, draft, bad-draft, retained-file, guarded-action, event-log, and lab controls exist with deterministic mock data. |
...
| Polling, focus, reconnect, route-transition, pre-mutation update checks | Proven | `src/shared/versionCheckClient.ts`, `src/components/AppShell.tsx`, `src/router/reactRouter.tsx`, `src/router/tanstackRouter.tsx`, compact browser and focused policy tests | Route transitions pass router location keys into `AppShell`; pre-mutation checks are covered by the guarded submit example. |
...
| React Router lazy route recovery | Proven | `src/router/reactRouter.tsx`, `src/shared/lazyRoute.ts`, `src/components/UpdateSurfaces.tsx`, `tests/version-skew.spec.ts` | Draft check, old-draft check, and retained-file failure paths are represented and covered. |
...
| Sensitive mutation metadata and idempotency | Proven | `src/shared/apiClient.ts`, `src/shared/api.ts`, `src/shared/sensitiveMutationGuard.ts`, `server/skew-server.ts`, `tests/api-client.spec.ts`, `tests/version-skew.spec.ts` | Draft submit, old-draft submit, and guarded-action mutations carry release/router/session metadata and idempotency where sensitive. |
| Autosave and draft restoration | Proven | `src/shared/workflowDraftStore.ts`, `src/workflows/SaveRefreshWorkflow.tsx`, `src/workflows/BadDraftWorkflow.tsx`, `tests/version-skew.spec.ts` | The save-refresh, bad-draft, and optional extra-draft paths are covered. |
| Old draft migration and incompatible fallback | Proven | `src/shared/workflowDraftStore.ts`, `src/workflows/BadDraftWorkflow.tsx`, `tests/version-skew.spec.ts` | v1-to-v2 migration and incompatible check-required fallback are implemented and tested. |
| Workflow chunk preloading | Proven | `src/shared/preloadWorkflowChunks.ts`, `src/pages/VersionSkewDebug.tsx`, `tests/version-skew.spec.ts` | The preload registry lists only chunks that map to visible learning steps; the debug table proof covers representative required and optional rows. |
...
| Security and privacy constraints | Proven | `README.md`, `server/mock-data.ts`, `src/shared/privacy.ts`, `src/shared/telemetry.ts`, `src/shared/auditLogClient.ts`, `server/skew-server.ts`, `src/components/AuditEventTable.tsx`, `tests/privacy.spec.ts`, `tests/project-contract.spec.ts` | Fake data, recursive telemetry/audit redaction, server audit persistence, and rendered audit-table masking are covered. The proof checks nested sensitive metadata while preserving safe counts. |
...
| Compact browser regression | Proven | `tests/version-skew.spec.ts` | The test file keeps the core browser guarantees readable: article path, lab controls, safe refresh, chunk fallback, guarded submit, retry key persistence, old draft fallback, TanStack recovery, event trace, and diagnostics. |
...
| UI/UX learning rebuild | Proven | `docs/audits/rebuild-audit.md`, `docs/audits/ui-evidence/*`, `tests/learning-path.spec.ts`, `tests/version-skew.spec.ts`, `tests/accessibility.spec.ts` | The learning path is much clearer and now has small browser specs that read like the three-example article. Reflow at 320px and normal-text contrast on primary learning/recovery surfaces are covered by focused browser tests. |
